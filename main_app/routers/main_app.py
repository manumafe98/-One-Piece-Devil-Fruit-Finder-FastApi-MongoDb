...
async def home_form(request: Request, search_element: str = Form(...)):
    ...
    response = requests.get(f"http://localhost:8000/devil_fruits/{devil_fruit_searched}")
...
async def add_form(request: Request, devil_fruit_name: str = Form(...), devil_fruit_type: str = Form(...), 
    ...
    response = requests.post("http://localhost:8000/devil_fruits", json=devil_fruit_params)
